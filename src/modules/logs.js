...
module.exports = ({ bot, knex, config, commands, hooks }) => {
  ...
  const logCmd = async (msg, args, _thread) => {
    ...
    if (customResponse && (customResponse.content || customResponse.file)) {
      channel.send(customResponse.content, customResponse.file);
    }

    const logUrl = await getLogUrl(thread);
    if (logUrl) {
      channel.send(`Open the following link to view the log for thread #${thread.thread_number}:\n<${addOptQueryStringToUrl(logUrl, args)}>`);
      return;
    }

    const logFile = await getLogFile(thread);
    if (logFile) {
      channel.send(`Download the following file to view the log for thread #${thread.thread_number}:`, logFile);
      return;
    }

    if (thread.status === THREAD_STATUS.OPEN) {
      channel.send(`This thread's logs are not currently available, but it's open at <#${thread.channel_id}>`);
      return;
    }
